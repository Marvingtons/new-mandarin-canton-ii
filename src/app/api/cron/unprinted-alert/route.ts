...
import { findUnprintedForAlert, markAlerted } from "@/lib/orders/repository";
...
export async function GET(request: Request): Promise<Response> {
  ...
  for (const order of due) {
    ...
    // makes "once per order" true.
    const claimed = await markAlerted(tenant.tenantId, order.id);
    if (!claimed) continue;
    ...
    if (result.sent) alerted++;
      ...
      console.error(
        `[alert] could not text the owner about ${order.orderNumber}: ${result.error}`,
      );
    }
  }

  return Response.json({ ok: true, found: due.length, alerted });
}
